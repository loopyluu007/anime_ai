# 项目结构规划

> **目标**: 定义拆分后的三大模块的目录结构  
> **版本**: v1.0

---

## 📋 目录

1. [整体结构](#整体结构)
2. [后端服务结构](#后端服务结构)
3. [移动端结构](#移动端结构)
4. [Web 端结构](#web-端结构)
5. [共享资源](#共享资源)

---

## 🏗️ 整体结构

```
anime_ai/
├── backend/                    # 后端服务
│   ├── api-gateway/           # API 网关
│   ├── services/              # 业务服务
│   ├── shared/                # 共享模块
│   └── infrastructure/        # 基础设施
├── mobile-app/                # 移动端应用
│   ├── lib/                   # Dart 代码
│   ├── android/               # Android 配置
│   └── ios/                   # iOS 配置
├── web-app/                   # Web 端应用
│   ├── lib/                   # Dart 代码（与移动端共享部分）
│   └── web/                   # Web 资源
├── shared/                    # 跨模块共享
│   ├── models/                # 数据模型定义
│   ├── api-spec/              # API 规范（OpenAPI）
│   └── docs/                  # 文档
└── docs/                      # 项目文档
```

---

## 🔧 后端服务结构

```
backend/
├── api-gateway/               # API 网关服务
│   ├── src/
│   │   ├── main.py           # 入口文件
│   │   ├── middleware/       # 中间件
│   │   │   ├── auth.py       # 认证中间件
│   │   │   ├── rate_limit.py # 限流中间件
│   │   │   └── cors.py       # CORS 中间件
│   │   ├── routes/           # 路由
│   │   │   └── gateway.py    # 网关路由
│   │   └── config/           # 配置
│   ├── requirements.txt
│   └── Dockerfile
│
├── services/
│   ├── agent-service/        # Agent 业务服务
│   │   ├── src/
│   │   │   ├── main.py
│   │   │   ├── api/          # API 路由
│   │   │   │   ├── conversations.py
│   │   │   │   ├── tasks.py
│   │   │   │   └── screenplays.py
│   │   │   ├── services/     # 业务逻辑
│   │   │   │   ├── conversation_service.py
│   │   │   │   ├── task_service.py
│   │   │   │   ├── screenplay_service.py
│   │   │   │   └── agent_service.py
│   │   │   ├── clients/       # 外部 API 客户端
│   │   │   │   └── glm_client.py
│   │   │   ├── models/       # 数据模型
│   │   │   │   ├── conversation.py
│   │   │   │   ├── task.py
│   │   │   │   └── screenplay.py
│   │   │   └── utils/         # 工具类
│   │   ├── requirements.txt
│   │   └── Dockerfile
│   │
│   ├── media-service/         # 媒体服务
│   │   ├── src/
│   │   │   ├── main.py
│   │   │   ├── api/           # API 路由
│   │   │   │   ├── images.py
│   │   │   │   ├── videos.py
│   │   │   ├── services/      # 业务逻辑
│   │   │   │   ├── image_service.py
│   │   │   │   ├── video_service.py
│   │   │   │   └── storage_service.py
│   │   │   ├── clients/       # 外部 API 客户端
│   │   │   │   ├── tuzi_client.py
│   │   │   │   └── gemini_client.py
│   │   │   ├── processors/    # 处理器
│   │   │   │   └── ffmpeg_processor.py
│   │   │   └── models/        # 数据模型
│   │   ├── requirements.txt
│   │   └── Dockerfile
│   │
│   └── data-service/          # 数据服务
│       ├── src/
│       │   ├── main.py
│       │   ├── api/           # API 路由
│       │   │   └── users.py
│       │   ├── services/      # 业务逻辑
│       │   │   └── user_service.py
│       │   ├── repositories/  # 数据访问层
│       │   │   ├── user_repository.py
│       │   │   ├── conversation_repository.py
│       │   │   └── task_repository.py
│       │   └── models/         # 数据模型
│       ├── requirements.txt
│       └── Dockerfile
│
├── shared/                     # 共享模块
│   ├── models/                 # 数据模型
│   │   ├── user.py
│   │   ├── conversation.py
│   │   ├── message.py
│   │   ├── task.py
│   │   └── screenplay.py
│   ├── utils/                  # 工具类
│   │   ├── logger.py
│   │   ├── exceptions.py
│   │   └── validators.py
│   └── config/                 # 配置
│       ├── database.py
│       └── redis.py
│
├── infrastructure/             # 基础设施
│   ├── database/               # 数据库
│   │   ├── migrations/        # 数据库迁移
│   │   │   └── 001_initial.sql  # 初始迁移（包含所有表结构和索引）
│   │   └── seeds/             # 种子数据
│   ├── docker/                # Docker 配置
│   │   ├── docker-compose.yml
│   │   ├── Dockerfile.gateway
│   │   ├── Dockerfile.agent
│   │   ├── Dockerfile.media
│   │   └── Dockerfile.data
│   └── nginx/                  # Nginx 配置
│       └── nginx.conf
│
├── tests/                      # 测试
│   ├── unit/                   # 单元测试
│   ├── integration/            # 集成测试
│   └── e2e/                    # 端到端测试
│
├── scripts/                    # 脚本
│   ├── migrate.py             # 数据迁移
│   ├── seed.py                # 种子数据
│   └── deploy.sh               # 部署脚本
│
├── .env.example                # 环境变量示例
├── .gitignore
└── README.md
```

---

## 📱 移动端结构 ⏸️ 已搁置

> **状态说明**: 移动端开发已标记为搁置，暂时不需要开发，未来很长一段时间内不会需要。但已编写的代码将保留在代码库中。

```
mobile-app/
├── lib/
│   ├── main.dart              # 应用入口
│   │
│   ├── core/                  # 核心模块
│   │   ├── api/               # API 客户端
│   │   │   ├── api_client.dart
│   │   │   ├── auth_client.dart
│   │   │   ├── conversation_client.dart
│   │   │   ├── task_client.dart
│   │   │   ├── screenplay_client.dart
│   │   │   └── media_client.dart
│   │   ├── storage/           # 本地存储
│   │   │   ├── local_storage.dart
│   │   │   ├── cache_manager.dart
│   │   │   └── hive_service.dart
│   │   ├── cache/             # 缓存
│   │   │   ├── image_cache.dart
│   │   │   └── video_cache.dart
│   │   ├── config/            # 配置
│   │   │   ├── app_config.dart
│   │   │   └── api_config.dart
│   │   └── utils/             # 工具类
│   │       ├── logger.dart
│   │       ├── validators.dart
│   │       └── formatters.dart
│   │
│   ├── features/              # 功能模块
│   │   ├── auth/              # 认证功能
│   │   │   ├── screens/
│   │   │   │   ├── login_screen.dart
│   │   │   │   └── register_screen.dart
│   │   │   ├── providers/
│   │   │   │   └── auth_provider.dart
│   │   │   └── models/
│   │   │       └── user.dart
│   │   │
│   │   ├── chat/              # 聊天功能
│   │   │   ├── screens/
│   │   │   │   ├── chat_screen.dart
│   │   │   │   └── conversation_list_screen.dart
│   │   │   ├── widgets/
│   │   │   │   ├── message_bubble.dart
│   │   │   │   ├── message_input.dart
│   │   │   │   └── screenplay_card.dart
│   │   │   ├── providers/
│   │   │   │   ├── chat_provider.dart
│   │   │   │   └── conversation_provider.dart
│   │   │   └── models/
│   │   │       ├── message.dart
│   │   │       └── conversation.dart
│   │   │
│   │   ├── screenplay/        # 剧本功能
│   │   │   ├── screens/
│   │   │   │   ├── screenplay_review_screen.dart
│   │   │   │   └── screenplay_detail_screen.dart
│   │   │   ├── widgets/
│   │   │   │   ├── scene_card.dart
│   │   │   │   ├── character_sheet_card.dart
│   │   │   │   └── progress_indicator.dart
│   │   │   ├── providers/
│   │   │   │   ├── screenplay_provider.dart
│   │   │   │   └── task_provider.dart
│   │   │   └── models/
│   │   │       ├── screenplay.dart
│   │   │       ├── scene.dart
│   │   │       └── character_sheet.dart
│   │   │
│   │   ├── gallery/           # 图库功能
│   │   │   ├── screens/
│   │   │   │   └── gallery_screen.dart
│   │   │   ├── widgets/
│   │   │   │   └── media_grid.dart
│   │   │   └── providers/
│   │   │       └── gallery_provider.dart
│   │   │
│   │   └── settings/          # 设置功能
│   │       ├── screens/
│   │       │   └── settings_screen.dart
│   │       ├── widgets/
│   │       │   └── settings_tile.dart
│   │       └── providers/
│   │           └── settings_provider.dart
│   │
│   ├── shared/                # 共享模块
│   │   ├── widgets/           # 通用组件
│   │   │   ├── loading_indicator.dart
│   │   │   ├── error_widget.dart
│   │   │   ├── empty_state.dart
│   │   │   └── video_player.dart
│   │   ├── utils/             # 工具类
│   │   │   ├── date_utils.dart
│   │   │   ├── string_utils.dart
│   │   │   └── file_utils.dart
│   │   └── themes/            # 主题
│   │       ├── app_theme.dart
│   │       └── colors.dart
│   │
│   └── platform/              # 平台特定（如有需要）
│
├── android/                    # Android 配置
│   ├── app/
│   │   ├── build.gradle
│   │   └── src/main/
│   │       ├── AndroidManifest.xml
│   │       └── kotlin/.../MainActivity.kt
│   └── build.gradle
│
├── ios/                       # iOS 配置
│   ├── Runner/
│   │   ├── Info.plist
│   │   └── AppDelegate.swift
│   └── Podfile
│
├── test/                      # 测试
│   ├── unit/
│   ├── widget/
│   └── integration/
│
├── assets/                    # 资源文件
│   ├── images/
│   ├── fonts/
│   └── icons/
│
├── pubspec.yaml
├── .gitignore
└── README.md
```

---

## 🌐 Web 端结构

```
web-app/
├── lib/
│   ├── main.dart              # 应用入口
│   │
│   ├── core/                  # 核心模块（与移动端共享）
│   │   ├── api/               # API 客户端（共享）
│   │   │   └── ...            # 与移动端相同
│   │   ├── storage/           # Web 存储适配
│   │   │   ├── web_storage_adapter.dart
│   │   │   └── indexed_db_adapter.dart
│   │   ├── config/            # 配置（共享）
│   │   │   └── ...            # 与移动端相同
│   │   └── utils/             # 工具类（共享）
│   │       └── ...            # 与移动端相同
│   │
│   ├── features/              # 功能模块（与移动端共享）
│   │   ├── auth/              # 认证功能
│   │   │   └── ...            # 与移动端相同
│   │   ├── chat/              # 聊天功能
│   │   │   └── ...            # 与移动端相同
│   │   ├── screenplay/        # 剧本功能
│   │   │   └── ...            # 与移动端相同
│   │   ├── gallery/           # 图库功能
│   │   │   └── ...            # 与移动端相同
│   │   └── settings/          # 设置功能
│   │       └── ...            # 与移动端相同
│   │
│   ├── shared/                # 共享模块（与移动端共享）
│   │   ├── widgets/           # 通用组件
│   │   │   └── ...            # 与移动端相同
│   │   ├── utils/             # 工具类
│   │   │   └── ...            # 与移动端相同
│   │   └── themes/            # 主题
│   │       └── ...            # 与移动端相同
│   │
│   └── web/                   # Web 特定
│       ├── adapters/          # Web 适配器
│       │   ├── storage_adapter.dart
│       │   ├── video_adapter.dart
│       │   ├── file_adapter.dart
│       │   └── image_picker_adapter.dart
│       ├── utils/             # Web 工具
│       │   ├── pwa_utils.dart
│       │   └── seo_utils.dart
│       └── workers/            # Web Workers
│           └── video_worker.dart
│
├── web/                       # Web 资源
│   ├── index.html
│   ├── manifest.json
│   ├── favicon.png
│   ├── icons/                 # PWA 图标
│   │   ├── icon-192.png
│   │   └── icon-512.png
│   └── assets/                # 静态资源
│       └── ...
│
├── test/                      # 测试
│   ├── unit/
│   ├── widget/
│   └── integration/
│
├── pubspec.yaml
├── .gitignore
└── README.md
```

---

## 🔄 共享资源

### 共享数据模型

```
shared/
├── models/                    # 数据模型定义（各端实现）
│   ├── user.dart              # 用户模型
│   ├── conversation.dart      # 对话模型
│   ├── message.dart           # 消息模型
│   ├── task.dart              # 任务模型
│   ├── screenplay.dart       # 剧本模型
│   ├── scene.dart             # 场景模型
│   └── character_sheet.dart   # 角色设定模型
│
├── api-spec/                  # API 规范
│   ├── openapi.yaml           # OpenAPI 规范
│   └── postman/               # Postman 集合
│       └── collection.json
│
└── docs/                      # 共享文档
    ├── api/                   # API 文档
    ├── architecture/          # 架构文档
    └── guides/                # 开发指南
```

### 代码共享策略

#### 1. 使用 Git Submodule（推荐）

```
mobile-app/
├── lib/
│   └── shared/ -> ../../shared/models/
└── pubspec.yaml

web-app/
├── lib/
│   └── shared/ -> ../../shared/models/
└── pubspec.yaml
```

#### 2. 使用 Dart Package

创建独立的 Dart 包：

```
shared_package/
├── lib/
│   ├── models/
│   ├── api/
│   └── utils/
├── pubspec.yaml
└── README.md
```

在 `mobile-app/pubspec.yaml` 和 `web-app/pubspec.yaml` 中引用：

```yaml
dependencies:
  shared_package:
    path: ../shared_package
```

#### 3. 使用符号链接（开发环境）

```bash
# 移动端
ln -s ../../shared/models lib/shared/models

# Web 端
ln -s ../../shared/models lib/shared/models
```

---

## 📦 依赖管理

### 后端依赖

```python
# requirements.txt (Python)
fastapi==0.104.1
uvicorn==0.24.0
sqlalchemy==2.0.23
psycopg2-binary==2.9.9
redis==5.0.1
pydantic==2.5.0
python-jose==3.3.0
python-multipart==0.0.6
```

### 移动端依赖 ⏸️ 已搁置

> **状态说明**: 移动端开发已标记为搁置，以下依赖信息仅供参考，代码已保留但不再继续开发。

```yaml
# pubspec.yaml
dependencies:
  flutter:
    sdk: flutter
  provider: ^6.1.1
  dio: ^5.4.0
  hive: ^2.2.3
  hive_flutter: ^1.1.0
  video_player: ^2.8.2
  chewie: ^1.7.4
  cached_network_image: ^3.3.1
  shared_preferences: ^2.2.2
  # ... 其他依赖
```

### Web 端依赖

```yaml
# pubspec.yaml
dependencies:
  flutter:
    sdk: flutter
  provider: ^6.1.1
  dio: ^5.4.0
  hive: ^2.2.3
  hive_flutter: ^1.1.0
  video_player: ^2.8.2
  cached_network_image: ^3.3.1
  shared_preferences: ^2.2.2
  # Web 特定依赖
  # ... 其他依赖
```

---

## 🔧 构建和部署

### 后端构建

```bash
# 构建 Docker 镜像
docker build -t directorai-backend:latest -f infrastructure/docker/Dockerfile.agent .

# 使用 Docker Compose
docker-compose up -d
```

### 移动端构建 ⏸️ 已搁置

> **状态说明**: 移动端开发已标记为搁置，以下构建命令仅供参考，代码已保留但不再继续开发。

```bash
# Android
flutter build apk --release
flutter build appbundle --release

# iOS
flutter build ios --release
```

### Web 端构建

```bash
# 构建 Web 应用
flutter build web --release

# 部署到服务器
# 将 build/web 目录部署到 Nginx 或 CDN
```

---

## 📝 配置文件示例

### 后端环境变量

```env
# .env
DATABASE_URL=postgresql://user:password@localhost:5432/directorai
REDIS_URL=redis://localhost:6379
JWT_SECRET=your-secret-key
GLM_API_KEY=your-glm-api-key
TUZI_API_KEY=your-tuzi-api-key
GEMINI_API_KEY=your-gemini-api-key
MINIO_ENDPOINT=localhost:9000
MINIO_ACCESS_KEY=minioadmin
MINIO_SECRET_KEY=minioadmin
```

### 移动端配置 ⏸️ 已搁置

> **状态说明**: 移动端开发已标记为搁置，以下配置信息仅供参考，代码已保留但不再继续开发。

```dart
// lib/core/config/api_config.dart
class ApiConfig {
  static const String baseUrl = 'https://api.directorai.com/api/v1';
  static const String wsUrl = 'wss://api.directorai.com/ws';
}
```

### Web 端配置

```dart
// lib/core/config/api_config.dart
class ApiConfig {
  static const String baseUrl = 'https://api.directorai.com/api/v1';
  static const String wsUrl = 'wss://api.directorai.com/ws';
}
```

---

## 📝 更新日志

### 2026-01-13
- ⏸️ 移动端开发标记为搁置
  - 将移动端开发状态标记为搁置
  - 明确说明暂时不需要开发，未来很长一段时间内不会需要
  - 已编写的代码保留在代码库中

---

**文档版本**: v1.1  
**最后更新**: 2026-01-13
