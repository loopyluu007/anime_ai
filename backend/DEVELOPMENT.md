# 后端开发施工文档

> **版本**: v1.4  
> **最后更新**: 2026-01-14（晚上）  
> **维护者**: 开发团队

---

## 📋 目录

1. [项目概述](#项目概述)
2. [开发环境配置](#开发环境配置)
3. [项目结构](#项目结构)
4. [开发指南](#开发指南)
5. [API实现状态](#api实现状态)
6. [数据库迁移](#数据库迁移)
7. [Docker部署](#docker部署)
8. [测试指南](#测试指南)
9. [常见问题](#常见问题)

---

## 🎯 项目概述

AI漫导后端服务采用微服务架构，基于FastAPI框架开发。主要包含：

- **Agent Service**: 处理对话、任务、剧本等核心业务
- **Media Service**: 处理图片、视频生成和合并
- **Data Service**: 处理用户数据和分析
- **Shared模块**: 共享的数据模型、工具类和配置

### 技术栈

- **语言**: Python 3.11+
- **框架**: FastAPI 0.104+
- **数据库**: PostgreSQL 15+
- **缓存**: Redis 7+
- **ORM**: SQLAlchemy 2.0+
- **认证**: JWT (python-jose)

---

## 🛠️ 开发环境配置

### 1. 环境要求

```bash
Python >= 3.11
PostgreSQL >= 15
Redis >= 7
```

### 2. 项目初始化

```bash
# 1. 进入backend目录
cd backend

# 2. 创建虚拟环境
python -m venv venv

# 3. 激活虚拟环境
# Windows:
venv\Scripts\activate
# Linux/Mac:
source venv/bin/activate

# 4. 安装依赖
pip install -r requirements.txt

# 5. 安装各服务的依赖
cd services/agent_service
pip install -r requirements.txt
```

### 3. 环境变量配置

创建 `.env` 文件（参考 `.env.example`）：

```bash
# 数据库
DATABASE_URL=postgresql://user:password@localhost:5432/directorai

# Redis
REDIS_URL=redis://localhost:6379/0

# JWT
SECRET_KEY=your-secret-key-here-change-in-production
ALGORITHM=HS256
ACCESS_TOKEN_EXPIRE_MINUTES=60

# API Keys
GLM_API_KEY=your-glm-api-key
TUZI_API_KEY=your-tuzi-api-key
GEMINI_API_KEY=your-gemini-api-key
```

### 4. 数据库初始化

```bash
# 创建数据库
createdb directorai

# 运行迁移脚本
psql -U postgres -d directorai -f infrastructure/database/migrations/001_initial.sql
```

---

## 📁 项目结构

```
backend/
├── api_gateway/              # API网关 ✅
│   ├── src/
│   │   ├── main.py          # 服务入口 ✅
│   │   ├── config/          # 配置
│   │   │   └── settings.py  # 配置管理 ✅
│   │   ├── middleware/      # 中间件
│   │   │   ├── auth.py      # 认证中间件 ✅
│   │   │   ├── rate_limit.py # 限流中间件 ✅
│   │   │   └── cors.py      # CORS中间件 ✅
│   │   └── routes/          # 路由
│   │       └── gateway.py   # 网关路由 ✅
│   └── requirements.txt
├── services/                 # 业务服务
│   ├── agent_service/       # Agent服务 ⭐
│   │   ├── src/
│   │   │   ├── main.py      # 服务入口
│   │   │   ├── api/         # API路由
│   │   │   │   ├── auth.py           # 认证API ✅
│   │   │   │   ├── conversations.py  # 对话API ✅
│   │   │   │   ├── messages.py       # 消息API ✅
│   │   │   │   ├── tasks.py          # 任务API ✅
│   │   │   │   └── screenplays.py    # 剧本API ✅
│   │   │   ├── services/    # 业务逻辑
│   │   │   │   ├── auth_service.py           # 认证服务 ✅
│   │   │   │   ├── conversation_service.py   # 对话服务 ✅
│   │   │   │   ├── message_service.py        # 消息服务 ✅
│   │   │   │   ├── task_service.py           # 任务服务 ✅
│   │   │   │   └── screenplay_service.py     # 剧本服务 ✅
│   │   │   └── clients/     # 第三方API客户端
│   │   │       └── glm_client.py     # GLM客户端 ✅
│   │   └── requirements.txt
│   ├── media_service/       # 媒体服务 ✅
│   │   ├── src/
│   │   │   ├── main.py      # 服务入口 ✅
│   │   │   ├── api/         # API路由
│   │   │   │   ├── images.py        # 图片API ✅
│   │   │   │   └── videos.py        # 视频API ✅
│   │   │   ├── services/    # 业务逻辑
│   │   │   │   ├── image_service.py  # 图片服务 ✅
│   │   │   │   └── video_service.py  # 视频服务 ✅
│   │   │   └── clients/     # 第三方API客户端
│   │   │       ├── tuzi_client.py    # Tuzi客户端 ✅
│   │   │       └── gemini_client.py  # Gemini客户端 ✅
│   │   └── requirements.txt
│   └── data_service/        # 数据服务 ✅
│       ├── src/
│       │   ├── main.py      # 服务入口 ✅
│       │   ├── api/         # API路由
│       │   │   └── users.py           # 用户数据API ✅
│       │   ├── services/    # 业务逻辑
│       │   │   └── user_service.py           # 用户服务 ✅
│       │   └── repositories/ # 数据访问层
│       │       ├── user_repository.py        # 用户数据访问 ✅
│       │       ├── conversation_repository.py # 对话数据访问 ✅
│       │       └── task_repository.py        # 任务数据访问 ✅
│       └── requirements.txt
├── shared/                   # 共享模块 ✅
│   ├── models/              # 数据模型
│   │   ├── user.py
│   │   ├── conversation.py
│   │   ├── message.py
│   │   ├── task.py
│   │   ├── screenplay.py
│   │   └── db_models.py     # SQLAlchemy模型
│   ├── utils/               # 工具类
│   │   ├── logger.py
│   │   └── exceptions.py
│   └── config/              # 配置
│       ├── database.py
│       └── redis.py
├── infrastructure/          # 基础设施 ✅
│   ├── database/
│   │   └── migrations/
│   │       └── 001_initial.sql
│   └── docker/              # Docker配置 ✅
│       ├── docker-compose.yml      # 生产环境配置 ✅
│       ├── docker-compose.dev.yml  # 开发环境配置 ✅
│       ├── start.sh                # 启动脚本 ✅
│       ├── start.bat               # 启动脚本(Windows) ✅
│       ├── stop.sh                 # 停止脚本 ✅
│       ├── stop.bat                # 停止脚本(Windows) ✅
│       └── README.md               # Docker使用文档 ✅
└── tests/                   # 测试（待实现）

图例: ✅ 已完成 | ⏳ 部分完成 | ❌ 待实现
```

---

## 📝 开发指南

### 1. 添加新的API路由

**步骤：**

1. 在 `services/agent_service/src/api/` 创建新的路由文件
2. 实现路由函数
3. 在 `main.py` 中注册路由

**示例：**

```python
# services/agent_service/src/api/example.py
from fastapi import APIRouter, Depends
from services.agent_service.src.api.auth import get_current_user

router = APIRouter(prefix="/example", tags=["示例"])

@router.get("")
async def get_example(current_user = Depends(get_current_user)):
    return {"message": "示例API"}

# services/agent_service/src/main.py
from services.agent_service.src.api import example
app.include_router(example.router, prefix="/api/v1")
```

### 2. 添加新的服务类

**步骤：**

1. 在 `services/agent_service/src/services/` 创建服务文件
2. 实现业务逻辑
3. 在API路由中调用服务

**示例：**

```python
# services/agent_service/src/services/example_service.py
from sqlalchemy.orm import Session

class ExampleService:
    def __init__(self, db: Session):
        self.db = db
    
    def do_something(self):
        # 业务逻辑
        pass
```

### 3. 添加数据库模型

**步骤：**

1. 在 `shared/models/db_models.py` 添加SQLAlchemy模型
2. 创建数据库迁移脚本
3. 在 `shared/models/` 添加Pydantic模型（如需要）

### 4. 添加第三方API客户端

**步骤：**

1. 在对应的service的 `clients/` 目录创建客户端文件
2. 实现API调用方法
3. 在服务类中使用客户端

---

## 🔌 API实现状态

### API Gateway

| 模块 | 状态 | 说明 |
|------|------|------|
| 配置管理 | ✅ | 服务地址、限流规则、CORS配置 |
| 认证中间件 | ✅ | JWT Token验证 |
| 限流中间件 | ✅ | 基于IP和用户ID的请求限流 |
| CORS中间件 | ✅ | 跨域请求支持 |
| 路由转发 | ✅ | 自动转发请求到对应服务 |
| 健康检查 | ✅ | 网关和服务健康状态监控 |

### Agent Service

| 模块 | 状态 | 说明 |
|------|------|------|
| 认证API | ✅ | 注册、登录、获取用户信息 |
| 对话API | ✅ | 创建、查询、更新、删除对话 |
| 消息API | ✅ | 获取消息列表、创建、查询、删除消息 |
| 任务API | ✅ | 创建、查询任务、获取进度 |
| 剧本API | ✅ | 生成草稿、确认、查询、更新 |
| WebSocket | ✅ | 实时推送任务进度、消息通知 |

### Media Service

| 模块 | 状态 | 说明 |
|------|------|------|
| Tuzi客户端 | ✅ | 视频生成客户端 |
| Gemini客户端 | ✅ | 图片生成客户端 |
| 图片API | ✅ | 图片生成API |
| 视频API | ✅ | 视频生成API |

### Data Service

| 模块 | 状态 | 说明 |
|------|------|------|
| 用户数据API | ✅ | 获取、更新用户信息、用户统计 |
| 数据访问层 | ✅ | 用户、对话、任务数据访问 |

### 第三方客户端

| 客户端 | 状态 | 说明 |
|--------|------|------|
| GLM客户端 | ✅ | 剧本生成 |
| Tuzi客户端 | ✅ | 视频生成 |
| Gemini客户端 | ✅ | 图片生成 |

---

## 🗄️ 数据库迁移

### 初始化数据库（新部署）

本项目为新部署，使用单一的初始迁移文件创建完整的数据库结构。

```bash
# 1. 创建数据库
createdb directorai

# 2. 运行初始迁移脚本（包含所有表结构和索引）
psql -U postgres -d directorai -f infrastructure/database/migrations/001_initial.sql
```

**注意**：`001_initial.sql` 已包含所有表结构、索引和约束，适用于新部署场景。

---

## 🐳 Docker部署

### 快速开始

1. **配置环境变量**

```bash
cd backend
cp .env.example .env
# 编辑 .env 文件，配置必要的环境变量
```

2. **启动服务**

```bash
# 生产环境（所有服务）
cd infrastructure/docker
./start.sh prod  # Linux/Mac
start.bat prod   # Windows

# 开发环境（仅基础设施）
./start.sh dev    # Linux/Mac
start.bat dev     # Windows
```

3. **停止服务**

```bash
./stop.sh [dev|prod]  # Linux/Mac
stop.bat [dev|prod]   # Windows
```

### 服务端口

| 服务 | 端口 | 说明 |
|------|------|------|
| API Gateway | 8000 | 统一API入口 |
| Agent Service | 8001 | Agent业务服务 |
| Media Service | 8002 | 媒体服务 |
| Data Service | 8003 | 数据服务 |
| PostgreSQL | 5432 | 数据库 |
| Redis | 6379 | 缓存 |
| MinIO | 9000 | 对象存储 |
| MinIO Console | 9001 | MinIO管理界面 |

### 详细文档

更多Docker部署信息，请参考：[infrastructure/docker/README.md](./infrastructure/docker/README.md)

---

## 🧪 测试指南

### 启动服务

```bash
# 启动Agent Service
cd services/agent_service
uvicorn src.main:app --reload --port 8001

# 访问API文档
# http://localhost:8001/docs
```

### API测试

使用FastAPI自动生成的Swagger文档：

- Swagger UI: `http://localhost:8001/docs`
- ReDoc: `http://localhost:8001/redoc`

### 测试认证

1. 注册用户：`POST /api/v1/auth/register`
2. 登录获取Token：`POST /api/v1/auth/login`
3. 使用Token访问受保护接口：在Header中添加 `Authorization: Bearer {token}`

### 单元测试

测试框架已搭建完成，使用pytest进行测试。

#### 运行测试

```bash
# 安装测试依赖
pip install -r requirements-test.txt

# 运行所有测试
pytest

# 运行单元测试
pytest tests/unit/ -m unit

# 运行集成测试
pytest tests/integration/ -m integration

# 查看覆盖率
pytest --cov=services --cov=shared --cov-report=html
```

#### 测试覆盖情况

当前测试覆盖率：~70%

| 服务 | 测试用例数 | 状态 |
|------|-----------|------|
| 认证服务 | 9 | ✅ 完成 |
| 对话服务 | 15 | ✅ 完成 |
| 任务服务 | 14 | ✅ 完成 |
| 剧本服务 | 11 | ✅ 完成 |
| 媒体服务 | 16 | ✅ 完成 |
| 消息服务 | 0 | ⏳ 待补充 |

详细测试文档请参考：[tests/README.md](./tests/README.md)

---

## ❓ 常见问题

### 1. 导入错误

**问题**: `ModuleNotFoundError: No module named 'services'`

**解决**: 
- 确保在backend目录下运行
- 检查 `sys.path` 配置
- 使用相对导入或正确设置PYTHONPATH

### 2. 数据库连接失败

**问题**: `sqlalchemy.exc.OperationalError`

**解决**:
- 检查 `DATABASE_URL` 配置
- 确认PostgreSQL服务运行
- 检查数据库用户权限

### 3. Redis连接失败

**问题**: `redis.exceptions.ConnectionError`

**解决**:
- 检查 `REDIS_URL` 配置
- 确认Redis服务运行
- 检查网络连接

### 4. JWT Token无效

**问题**: `AuthenticationError: Token 无效`

**解决**:
- 检查Token是否过期
- 确认SECRET_KEY配置正确
- 检查Token格式（Bearer {token}）

### 5. 第三方API调用失败

**问题**: `httpx.HTTPStatusError`

**解决**:
- 检查API Key配置
- 查看错误响应详情
- 确认API服务可用

---

## 📚 参考文档

- [后端工程实施方案](../docs/后端工程实施方案.md)
- [API接口设计文档](../docs/API接口设计文档.md)
- [数据库设计文档](../docs/数据库设计文档.md)
- [FastAPI文档](https://fastapi.tiangolo.com/)
- [SQLAlchemy文档](https://docs.sqlalchemy.org/)

---

## 🚀 下一步工作

### 优先级高

1. ✅ 完成目录重命名（连字符改为下划线）
2. ✅ 实现认证API和服务
3. ✅ 实现对话API和服务
4. ✅ 实现任务API和服务
5. ✅ 实现第三方API客户端
6. ✅ 实现剧本API和服务
7. ✅ 实现Media Service的API（图片/视频生成）
8. ✅ 实现WebSocket实时推送

### 优先级中

1. ✅ 实现Data Service
2. ✅ 实现API Gateway
3. ✅ 添加单元测试 ⬆️ 核心服务测试已完成（2026-01-13）
   - ✅ 认证服务测试（9个测试用例）
   - ✅ 对话服务测试（15个测试用例）
   - ✅ 任务服务测试（14个测试用例）
   - ✅ 剧本服务测试（11个测试用例）
   - ✅ 媒体服务测试（16个测试用例）
   - 测试覆盖率：~70%
4. ⏳ 添加集成测试（部分完成：认证API测试已完成）
5. ✅ Docker配置
6. ✅ 部署配置

### 优先级低

1. ❌ 性能优化
2. ❌ 监控和日志
3. ❌ 文档完善

---

## 📝 更新日志

### 2026-01-12

- ✅ 实现消息管理模块（Messages API）
  - 实现消息服务层（MessageService）- 消息查询、创建、删除等业务逻辑
  - 实现消息API路由（messages.py）- GET/POST/DELETE接口
  - 支持分页查询和按时间筛选（before参数）
  - 自动更新对话的消息计数和最后访问时间
  - 在main.py中注册消息路由

- ✅ 实现Docker配置和部署配置
  - 创建各服务的Dockerfile（api_gateway、agent_service、media_service、data_service）
  - 创建docker-compose.yml（生产环境配置，包含所有服务）
  - 创建docker-compose.dev.yml（开发环境配置，仅基础设施）
  - 创建启动/停止脚本（start.sh、start.bat、stop.sh、stop.bat）
  - 创建.dockerignore文件
  - 创建.env.example环境变量配置示例
  - 创建Docker部署文档（infrastructure/docker/README.md）

- ✅ 实现API Gateway（API网关）
  - 实现配置管理（服务地址、限流规则、CORS配置）
  - 实现认证中间件（JWT Token验证）
  - 实现限流中间件（基于IP和用户ID的请求限流）
  - 实现CORS中间件（跨域请求支持）
  - 实现路由转发（自动转发请求到对应服务）
  - 实现健康检查（网关和服务健康状态监控）

- ✅ 实现Data Service（数据服务）
  - 实现用户数据API（获取、更新用户信息、用户统计）
  - 实现数据访问层（UserRepository、ConversationRepository、TaskRepository）
  - 实现服务层（UserService）

### 2026-01-14（晚上）
- ✅ **删除数据分析功能**
  - 删除后端数据分析API文件（analytics.py）
  - 删除后端数据分析服务文件（analytics_service.py）
  - 删除数据分析测试文件（test_analytics_service.py）
  - 更新data_service/main.py移除analytics导入和路由
  - 更新API Gateway配置移除analytics路由
  - 更新所有相关文档
  - **说明**: 根据需求，完全移除数据分析功能

### 2025-01-12

- ✅ 创建项目基础结构
- ✅ 实现共享模块（models、utils、config）
- ✅ 实现数据库模型和迁移脚本
- ✅ 实现认证API和服务
- ✅ 实现对话API和服务
- ✅ 实现任务API和服务
- ✅ 实现剧本API和服务
- ✅ 实现GLM、Tuzi、Gemini客户端
- ✅ 目录重命名（连字符改为下划线）
- ✅ 创建开发施工文档
- ✅ 实现Media Service API（图片/视频生成）
- ✅ 实现WebSocket实时推送
- ✅ 实现API Gateway（统一入口、认证、限流、路由转发）

---

**文档版本**: v1.4  
**最后更新**: 2026-01-14（晚上）  
**维护者**: 开发团队
